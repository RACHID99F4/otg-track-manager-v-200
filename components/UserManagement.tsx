...
import { User, UserPermissions, ModulePermissions, AppConfig } from '../types.js';
import { Users, Plus, Trash2, Camera, ShieldCheck, ShieldAlert, Key, UserCircle, BadgeCheck, Lock, UserPlus, Info, CheckCircle2, XCircle, Eye, Edit2 } from 'lucide-react';
import { ConfirmDialog } from './ConfirmDialog.js';
import { DEFAULT_USER_PHOTO } from '../constants.js';
...
export const UserManagement: React.FC<UserManagementProps> = ({ users, onUpdateUsers, currentUser, config }) => {
  ...
  const [newUser, setNewUser] = useState<Partial<User>>({
    ...
    permissions: {
      ...
      calculator: true,
      otgRepairs: { read: false, edit: false, delete: false },
      custom: false,
      admin: false
    }
  });
  ...
  const handleAddUser = async () => {
    if ((!isAdding && !editingUser) || (isAdding && (!newUser.name || !newUser.username || !newUser.password))) {
      if (isAdding) alert("Veuillez remplir tous les champs obligatoires.");
      return;
    }

    if (isAdding && users.some(u => u.username === newUser.username)) {
  ...
  const togglePermission = (userId: string, permission: keyof UserPermissions, subPermission?: keyof ModulePermissions) => {
    ...
    const updatedUsers = users.map(u => {
      if (u.id === userId) {
        const currentPerm = u.permissions[permission];

        let updatedPerm: any;
        if (subPermission && typeof currentPerm === 'object') {
          updatedPerm = {
            ...currentPerm,
            [subPermission]: !currentPerm[subPermission]
          };
        } else {
          updatedPerm = !currentPerm;
        }
  ...
  const confirmHandleDelete = () => {
    if (confirmDelete) {
      onUpdateUsers(users.filter(u => u.id !== confirmDelete));
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-20 max-w-[1400px] mx-auto">
      <ConfirmDialog
        ...
        message="Voulez-vous vraiment supprimer ce compte collaborateur ? Toutes les données associées resteront mais l'accès sera révoqué définitivement."
      ...
      {(isAdding || editingUser) && (
        <div className="bg-white dark:bg-slate-900 border-2 border-primary/20 rounded-[3rem] p-10 shadow-2xl animate-fade-in mb-12">
          <div className="flex items-center justify-between mb-10">
            <h3 className="text-xl font-black text-secondary dark:text-blue-200 uppercase tracking-widest flex items-center gap-3">
              <UserCircle className="w-6 h-6 text-primary" /> {isAdding ? 'Nouveau profil utilisateur' : 'Modifier le profil'}
            </h3>
            <button onClick={() => { setIsAdding(false); setEditingUser(null); }} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
            <div className="flex flex-col items-center gap-6 text-center">
              <div className="relative group">
                <img src={isAdding ? newUser.photo : editingUser?.photo} className="w-40 h-40 rounded-[2.5rem] object-cover border-8 border-slate-50 dark:border-slate-800 shadow-xl" alt="Avatar Preview" />
                <label className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition-all cursor-pointer backdrop-blur-sm">
                  ...
                  <input type="file" accept="image/*" onChange={(e) => handlePhotoUpload(e, isAdding, editingUser?.id)} className="hidden" />
                </label>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (isAdding) setNewUser(prev => ({ ...prev, name: e.target.value }));
                      else if (editingUser) setEditingUser(prev => prev ? ({ ...prev, name: e.target.value }) : null);
                    }}
                    className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 dark:border-slate-800 font-black text-sm focus:border-primary outline-none bg-slate-50 dark:bg-slate-800"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (isAdding) setNewUser(prev => ({ ...prev, role: val }));
                      else if (editingUser) {
                        if (editingUser.id === currentUser?.id && val === 'user' && currentUser.role === 'admin') {
                          alert("Vous ne pouvez pas retirer vos propres droits administrateur.");
                          return;
                        }
                        setEditingUser(prev => prev ? ({ ...prev, role: val }) : null);
                      }
                    }}
                    className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 dark:border-slate-800 font-black text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-primary"
                  >
              ...
              {isAdding && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* Standard Boolean Permissions */}
                    {(['dashboard', 'calculator', 'custom', 'admin'] as (keyof UserPermissions)[]).map(p => {
                      const isEnabled = newUser.permissions?.[p];
                      const label = typeof p === 'string' ? p.toUpperCase() : String(p);
                      return (
                        <button
                          key={p}
                          type="button"
                          ...
                          className={`flex items-center justify-between px-5 py-4 rounded-2xl border-2 font-black text-[10px] tracking-tight transition-all ${isEnabled
                            ? 'bg-primary/5 border-primary text-primary shadow-sm'
                            : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-400'
                            }`}
                        >
                          {label}
                          {isEnabled ? <CheckCircle2 className="w-4 h-4" /> : <XCircle className="w-4 h-4 opacity-30" />}
                        </button>
                      );
                    })}

                    {/* Granular Module Permissions */}
                    {(['inventory', 'otgRepairs'] as (keyof UserPermissions)[]).map(p => {
                      const modulePerms = newUser.permissions?.[p] as ModulePermissions;
                      const label = typeof p === 'string' ? p.toUpperCase() : String(p);
                      return (
                        <div key={p} className="flex flex-col gap-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border-2 border-slate-100 dark:border-slate-700">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{label} ACCESS</span>
                          <div className="flex gap-2">
                            {(['read', 'edit', 'delete'] as (keyof ModulePermissions)[]).map(sub => (
                              <button
                                key={sub}
                                type="button"
                                onClick={() => setNewUser(prev => {
                                  const currentModule = prev.permissions![p] as ModulePermissions;
                                  return {
                                    ...prev,
                                    permissions: {
                                      ...prev.permissions!,
                                      [p]: { ...currentModule, [sub]: !currentModule[sub] }
                                    }
                                  };
                                })}
                                className={`flex-1 p-2 rounded-xl border font-black text-[8px] transition-all flex flex-col items-center gap-1 ${modulePerms[sub]
                                  ? 'bg-primary/10 border-primary text-primary'
                                  : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-300'
                                  }`}
                              >
                                {sub === 'read' && <Eye className="w-3 h-3" />}
                                {sub === 'edit' && <Edit2 className="w-3 h-3" />}
                                {sub === 'delete' && <Trash2 className="w-3 h-3" />}
                                {sub.toUpperCase()}
                              </button>
                            ))}
                          </div>
          ...
          <div className="mt-12 pt-10 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-6">
            <button onClick={() => { setIsAdding(false); setEditingUser(null); }} className="px-10 py-4 font-black text-slate-400 hover:text-red-500 transition-colors uppercase text-xs tracking-widest">Annuler</button>
            <button onClick={handleAddUser} className="px-12 py-5 bg-secondary dark:bg-blue-600 text-white font-black rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all uppercase text-xs tracking-widest">{isAdding ? 'Confirmer la création' : 'Enregistrer les modifications'}</button>
          </div>
        </div>
      )}
              ...
              <div className="p-8 pb-4 flex items-start justify-between">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={() => { setEditingUser(user); setIsAdding(false); }} className="p-2 bg-slate-50 dark:bg-slate-800 rounded-xl hover:bg-primary hover:text-white transition-all border border-slate-100 dark:border-slate-700 shadow-sm">
              ...
              <div className="px-8 flex-1">
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {(['dashboard', 'calculator', 'custom', 'admin'] as (keyof UserPermissions)[]).map(p => (
                      <button
                        ...
                        className={`flex items-center justify-between p-2.5 rounded-xl border transition-all ${user.permissions[p]
                      ...
                      >
                        <span className="text-[8px] font-black uppercase tracking-tighter truncate flex-1 text-left">{String(p)}</span>
                        {user.permissions[p] ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="mt-3 flex flex-col gap-2">
                    {(['inventory', 'otgRepairs'] as (keyof UserPermissions)[]).map(p => {
                      const modulePerms = user.permissions[p] as ModulePermissions;
                      const label = String(p).toUpperCase();
                      return (
                        <div key={p} className="p-2.5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl">
                          <div className="text-[7px] font-black text-slate-400 uppercase mb-1.5 flex justify-between">
                            <span>{label}</span>
                          </div>
                          <div className="flex gap-1.5">
                            {(['read', 'edit', 'delete'] as (keyof ModulePermissions)[]).map(sub => (
                              <button
                                key={sub}
                                onClick={() => togglePermission(user.id, p, sub)}
                                className={`flex-1 flex flex-col items-center gap-0.5 p-1 rounded-lg border transition-all text-center ${modulePerms[sub]
                                  ? 'bg-primary/5 border-primary/30 text-primary'
                                  : 'bg-transparent border-slate-100 dark:border-slate-800 text-slate-300 opacity-30'
                                  }`}
                                title={`${label} - ${sub.toUpperCase()}`}
                              >
                                {sub === 'read' && <Eye className="w-3 h-3" />}
                                {sub === 'edit' && <Edit2 className="w-3 h-3" />}
                                {sub === 'delete' && <Trash2 className="w-3 h-3" />}
                                <span className="text-[6px] font-black uppercase text-[5px]">{sub}</span>
                              </button>
                            ))}
                          </div>
                        </div>
