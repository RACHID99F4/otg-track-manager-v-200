...
import { OTGRepair, AppConfig, User } from '../types.js';
import { Search, Edit2, Trash2, Filter, Wrench, User as UserIcon, Calendar, AlertCircle, Hash } from 'lucide-react';
...
export const OTGRepairTable: React.FC<OTGRepairTableProps> = ({ repairs, onEdit, onDelete, config, currentUser, onConfirmDelete }) => {
              ...
              <tr key={repair.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all text-sm group">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {(currentUser?.role === 'admin' || currentUser?.permissions.otgRepairs.edit) && (
                      <button type="button" onClick={() => onEdit(repair)} title="Modifier" className="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-all">
                        <Edit2 className="w-4 h-4" />
                      </button>
                    )}
                    {(currentUser?.role === 'admin' || currentUser?.permissions.otgRepairs.delete) && (
                      <button type="button" onClick={() => onConfirmDelete(repair.id)} title="Supprimer" className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-all">
