...
import { AppConfig, ColumnConfig, CustomFieldDefinition } from '../types.js';
import { Palette, Image, Layout, ArrowUp, ArrowDown, Eye, EyeOff, Plus, Trash2, Cpu, FileText, Truck, Mail, BellRing, User } from 'lucide-react';
...
export const CustomSettings: React.FC<CustomSettingsProps> = ({ config, onUpdate }) => {
  ...
  const addConducteur = () => {
    if (!newConducteur || config.conducteurs.includes(newConducteur.toUpperCase())) return;
    onUpdate({
      ...config,
      conducteurs: [...config.conducteurs, newConducteur.toUpperCase()]
    });
    setNewConducteur('');
  };

  const removeConducteur = (c: string) => {
    onUpdate({
      ...config,
      conducteurs: config.conducteurs.filter(item => item !== c)
    });
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-fade-in pb-20">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 p-8 space-y-8 transition-colors">
        <div className="flex items-center gap-4">
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-2xl"><User className="w-8 h-8" /></div>
          <div><h2 className="text-2xl font-black text-secondary dark:text-blue-200">Gestion des Conducteurs</h2><p className="text-gray-500 dark:text-slate-400 font-medium">Gérez la liste des opérateurs pour les réparations OTG.</p></div>
        </div>
        <div className="space-y-6">
          <div className="flex gap-4">
            <input
              ...
              placeholder="NOM DU NOUVEAU CONDUCTEUR"
            ...
            />
            <button onClick={addConducteur} className="px-8 py-4 bg-primary text-white font-black rounded-2xl shadow-lg shadow-primary/20 flex items-center gap-2 hover:scale-105 active:scale-95 transition-all">
              <Plus className="w-5 h-5" /> AJOUTER
            </button>
          </div>
          <div className="flex flex-wrap gap-3">
            {(config.conducteurs || []).map(c => (
              <div key={c} className="flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 group hover:border-blue-200 dark:hover:border-blue-500 transition-all">
                <span className="font-black text-secondary dark:text-slate-200 text-xs">{c}</span>
                <button
                  onClick={() => showConfirm(
                    "Supprimer le conducteur ?",
                    `Êtes-vous sûr de vouloir supprimer le conducteur "${c}" ?`,
                    () => {
                      removeConducteur(c);
                      closeConfirm();
                    }
              ...
              <div key={s} className="flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 group hover:border-amber-200 dark:hover:border-amber-500 transition-all">
                ...
                <button
                  onClick={() => showConfirm(
                    ...
                    `Êtes-vous sûr de vouloir supprimer le fournisseur "${s}" ?`,
                    () => {
                      removeSupplier(s);
                      closeConfirm();
                    }
              ...
              <div key={m} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 group transition-colors">
                ...
                <button
                  onClick={() => showConfirm(
                    ...
                    `Êtes-vous sûr de vouloir supprimer la machine "${m}" ?`,
                    () => {
                      removeMachine(m);
                      closeConfirm();
                    }
                  )}
                  className="p-2 text-red-300 hover:text-red-500 hover:bg-white dark:hover:bg-slate-700 rounded-lg transition-all"
                >
            ...
            {config.customFieldDefinitions.map(f => (
              <div key={f.id} className="flex items-center justify-between p-4 bg-indigo-50/30 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-900 rounded-2xl transition-colors">
                ...
                <button
                  onClick={() => showConfirm(
                    "Supprimer le champ ?",
                    `Êtes-vous sûr de vouloir supprimer le champ personnalisé "${f.label}" ?`,
                    () => {
