...
import { InventoryItem, ItemStatus, ColumnConfig, AppConfig, User } from '../types.js';
import { calculateStatus, formatDate } from '../utils.js';
...
export const InventoryTable: React.FC<InventoryTableProps> = ({ items, machineFilter, onEdit, onDelete, columnConfig, config, currentUser, onConfirmDelete }) => {
              ...
              <tr key={item.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all text-sm group">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {(currentUser?.role === 'admin' || currentUser?.permissions.inventory.edit) && (
                      <button type="button" onClick={(e) => { e.stopPropagation(); onEdit(item); }} className="p-2.5 text-blue-600 dark:text-blue-400 bg-white dark:bg-slate-800 hover:bg-blue-600 hover:text-white rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 transition-all">
                        <Edit2 className="w-4 h-4" />
                      </button>
                    )}
                    {(currentUser?.role === 'admin' || currentUser?.permissions.inventory.delete) && (
                      <button type="button" onClick={(e) => { e.stopPropagation(); onConfirmDelete(item.id); }} className="p-2.5 text-red-500 bg-white dark:bg-slate-800 hover:bg-red-500 hover:text-white rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 transition-all">
